...
class AdaptedCLIP(nn.Module):
    def __init__(
            ...
            image_adapt_layers: list = [0, 1, 2, 3, 4, 5],
        ...
        self.image_adapt_layers = image_adapt_layers
        self.i_w = image_adapt_weight
        self.levels = levels

        # Create adapters for specified image layers
        layer_adapters = nn.ModuleDict({
            str(layer_idx): SimpleAdapter(1024, 1024)
            for layer_idx in image_adapt_layers
        })
    ...
    def forward(self, x):
        ...
        for i in range(24):
            ...
            # Check if current layer is in adapter list
            if i in self.image_adapt_layers:
                adapt_out = self.image_adapter["layer_adapters"][str(i)](x)
                adapt_out = (
                        adapt_out
